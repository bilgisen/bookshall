...
import { z } from "zod";
import { auth } from "@/lib/auth/better-auth"; // better-auth server helper
...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      logger.error("Error parsing request body", { error: errorMessage });
      return NextResponse.json(
    ...
    try {
      ...
      logger.error("Database error during update", { error: errorMessage });
      return NextResponse.json(
        {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    logger.error("Unexpected error in EPUB callback", { error: errorMessage });
    return NextResponse.json(
      {
